...
function logTexte () {
    ...
    const texte = document.querySelector("#cLa");
    texte.innerHTML += document.querySelector("#afficheTexte").value;
}
let i = 0;
function creerUtilisateur () {
    i = i +1;
    const ligne = document.querySelector("#tbody");
    let valeurGenre = "";
    if (document.querySelector("#homme").checked) {valeurGenre = "H"}
    else if (document.querySelector("#femme").checked) {valeurGenre = "F"}

    ligne.innerHTML +=
        '<tr id="nouvelleLigne">'
        + '<td>' + i + '</td>'
        + '<td>' + document.querySelector("#nom").value +'</td>'
        + '<td>' + document.querySelector("#prenom").value + '</td>'
        + '<td>' + valeurGenre + '</td>'
        + '<td>' + document.querySelector("#dateNaissance").value + '</td>'
        + '<td>' + document.querySelector("#lieuNaissance").value + '</td>'
        + '<td>' + document.querySelector("#departementNaissance").value + '</td>'
        + '<td>' + document.querySelector("#numRue").value + ' ' + document.querySelector("#nomRue").value + ' ' + document.querySelector("#codePostal").value + ' ' + document.querySelector("#ville").value + '</td>'
        + '<td>' + '<button onclick="supprimerUtilisateur()">Supprimer</button> ' + '</td>'

}

function supprimerUtilisateur () {
    const tableauActuel = document.querySelector("#tableau");
    const idSup = document.querySelector("#supprimeLigne").value;
    let valeurId = document.querySelector("#id");
    tableauActuel.deleteRow(valeurId=idSup)
}
