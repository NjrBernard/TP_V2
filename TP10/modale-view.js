...
function creerUtilisateur() {
    const tbody = document.getElementById('contenuTableau');

    const prenom = document.querySelector('input[name="prenom"]').value;
    const nom = document.querySelector('input[name="nom"]').value;
    const dateNaissance = document.querySelector('input[name="dateNaissance"]').value;
    const lieuNaissance = document.querySelector('input[name="lieuNaissance"]').value;
    const adresse = document.querySelector('input[name="adresse"]').value;

    const tr = document.createElement('tr');
    const ajoutCellule = (text) => { const td = document.createElement('td'); td.textContent = text ?? ''; return td; };
    const boutonSupr = '🗑️';
    const id = tbody.querySelectorAll('tr').length + 1;
    tr.appendChild(ajoutCellule(id));
    tr.appendChild(ajoutCellule(prenom));
    tr.appendChild(ajoutCellule(nom));
    tr.appendChild(ajoutCellule(dateNaissance));
    tr.appendChild(ajoutCellule(lieuNaissance));
    tr.appendChild(ajoutCellule(adresse));
    tr.appendChild(ajoutCellule(boutonSupr));
